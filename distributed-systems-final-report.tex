...
\textbf{Product type.}
The project implements a \emph{distributed peer-to-peer application} for sensor \emph{management and replication} in smart-city scenarios.
Concretely, it combines:
\begin{itemize}
  \item a node runtime service (Python process) handling sensor ingestion, replication, membership, failure detection, and protocol logic;
  \item inter-node TCP-based communication services (custom protocol);
  \item an optional observability layer for real-time monitoring.
\end{itemize}

\textbf{Use case description.}
The software manages a cluster of sensor nodes that ingest readings and disseminate updates to peers, so that each node converges toward a coherent global view (eventual consistency with deterministic LWW conflict resolution).

In the current demo, sensor streams are simulated; however, the ingestion layer is designed so that real sensor sources can be added through dedicated adapters with no architectural changes to replication logic.

Typical users are smart-city monitoring stakeholders with two main human interaction profiles:
\begin{itemize}
  \item \emph{observational interaction} (control-room operators): inspect live sensor values and cluster status;
  \item \emph{operational interaction} (technical operators): start/stop nodes, apply configuration changes, and run recovery or fault-handling procedures.
\end{itemize}

Machine-to-machine interaction is \emph{continuous and periodic}: heartbeat exchange, gossip rounds, push-pull synchronization, and recovery synchronization after outages.

Interaction channels and devices:
\begin{itemize}
  \item terminal/CLI for deployment and configuration;
  \item monitoring clients and automated tests for system observability;
  \item desktop/laptop web browser for optional dashboard-based monitoring.
\end{itemize}

Data handling and roles:
\begin{itemize}
  \item nodes store replicated sensor records, membership/liveness metadata, topology snapshots, and introspection counters/events (node-local, runtime state);
  \item optional log files provide operational traces;
  \item main roles include: \emph{node process} (autonomous participant), \emph{cluster operator/developer}, and \emph{observer client} (UI/tests/tools).
\end{itemize}

\textbf{Why distribution is needed.}
Distribution is a core requirement, not an implementation detail:
\begin{itemize}
  \item \emph{fault tolerance and resilience}: no single coordinator; nodes can continue under crashes/restarts and temporary partitions;
  \item \emph{eventual convergence at scale}: each node contributes updates and receives others through push/pull plus gossip dissemination;
  \item \emph{resource and workload sharing}: sensing, merge, and communication work are spread across peers;
  \item \emph{realistic decentralized environments}: smart-city infrastructures are geographically distributed by nature, so data is produced at the edge and synchronized across independent participants.
\end{itemize}
