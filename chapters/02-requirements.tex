...
Each node shall be able to host one or more local sensor sources and ingest their readings into the distributed system state.
...
The system shall propagate sensor updates among nodes through a decentralized gossip mechanism so that readings produced on one node become visible on other reachable nodes.
...
The system shall allow nodes to discover peers and maintain a distributed view of cluster membership.
...
\textbf{Acceptance criterion.}
If a known peer becomes unreachable for long enough, at least one observing node eventually reports a degraded or failed status for that peer in its membership/introspection output.

\medskip
\textbf{FR7 -- Recovery after crashes or temporary disconnections.}
The system shall allow a node that restarts or reconnects after temporary unavailability to recover the replicated state needed to rejoin normal operation, including full-state synchronization when incremental information is insufficient.

\textbf{Acceptance criterion.}
After a node crash or temporary isolation, once communication is restored the recovering node eventually exposes a replicated state aligned with the rest of the reachable cluster.
...
\textbf{Acceptance criterion.}
A client can query documented HTTP endpoints and obtain structured snapshots of the node's current distributed-system view without modifying runtime state.

\medskip
\textbf{FR9 -- Human-oriented monitoring support.}
...
\textbf{Acceptance criterion.}
An observer can inspect live cluster information from a web browser by connecting to the exposed read-only API of a node.

\medskip
\textbf{FR10 -- Push-pull anti-entropy exchange.}
...
\textbf{Acceptance criterion.}
During normal operation, pairs of reachable peers periodically exchange synchronization messages in both directions, and each node can obtain updates it does not yet store.

\medskip
\textbf{FR11 -- Phi Accrual failure suspicion output.}
The membership subsystem shall expose failure suspicion derived from a Phi Accrual Failure Detector, allowing operators to distinguish healthy, suspect, and recovering peers.

\textbf{Acceptance criterion.}
If heartbeat arrival patterns from a peer degrade or stop, phi-based suspicion eventually crosses a configured threshold and the peer is reported as suspect/faulty in membership/introspection output.

\medskip
\textbf{FR12 -- Full-state rejoin synchronization.}
When a node rejoins after crash, restart, or long partition, it shall support full-state synchronization from reachable peers to rebuild a coherent replica before resuming steady-state convergence.

\textbf{Acceptance criterion.}
After prolonged unavailability, a rejoining node can request and apply a full sensor-state snapshot and eventually align with the reachable cluster state.

\medskip
\textbf{FR13 -- Reproducible multi-node orchestration.}
The project shall provide an executable multi-node deployment configuration for experiments and demonstrations (e.g., Docker Compose topology with multiple peers).

\textbf{Acceptance criterion.}
Using repository-provided orchestration artifacts, an operator can start a predefined multi-node topology and observe ingestion, replication, failure suspicion, and recovery behavior.

\medskip
\textbf{FR14 -- Comparative technology discussion artifact.}
Project documentation shall include a comparative discussion against common distributed messaging/streaming frameworks (e.g., MQTT, ZeroMQ, Kafka, Redis Streams), with explicit trade-offs.

\textbf{Acceptance criterion.}
The final report contains a section that contrasts the implemented peer-to-peer design with the selected frameworks along consistency, scalability, and fault-tolerance dimensions.
...
\textbf{Acceptance criterion.}
Using the documented setup, an operator can start a predefined topology and reproduce representative convergence and recovery experiments without relying on external managed services.

\medskip
\textbf{NFR6 -- Extensibility of sensor sources.}
The system shall be open to additional sensor types without changing the conceptual role of the rest of the distributed runtime.
...
\textbf{Acceptance criterion.}
The documented deployment can be instantiated with multiple nodes and still provides replicated state and observability functions for the whole experiment.

\subsection{Implementation Constraints}
\label{implementation-constraints}

\textbf{IR1 -- Self-contained deployment.}
The project shall be executable in a self-contained way on commodity machines used for development and evaluation, without requiring paid or institution-managed cloud infrastructure.

\textbf{Justification.}
This constraint follows from course-project reproducibility and portability requirements.

\textbf{Acceptance criterion.}
The system can be started locally with the provided repository artifacts and documented dependencies.

\medskip
\textbf{IR2 -- Reproducible multi-node setup.}
The project shall provide a reproducible way to instantiate representative multi-node deployments for demonstrations and validation.

\textbf{Justification.}
The report and validation activities require repeatable distributed scenarios, including fault-injection experiments.

\textbf{Acceptance criterion.}
An operator can launch a predefined topology from the repository and repeat the same experiment configuration with consistent system roles and interfaces.
