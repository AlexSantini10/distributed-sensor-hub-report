...
The system shall propagate sensor updates among nodes so that readings produced on one node become visible on other reachable nodes.
...
\medskip
\textbf{FR4 -- Deterministic conflict resolution.}
The system shall resolve concurrent or conflicting updates in a deterministic way so that replicas can converge to the same winning value for each sensor record.
...
\medskip
\textbf{FR6 -- Liveness observation.}
...
The system shall allow a node that restarts or reconnects after temporary unavailability to recover the replicated state needed to rejoin normal operation.
